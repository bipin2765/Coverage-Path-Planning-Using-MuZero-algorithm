...
def train(network_model, config, replay_buffer):
    ...
    with tf.GradientTape() as tape:
        ...
        loss = 0
        optimizer = tf.keras.optimizers.Adam(learning_rate=config['train']['learning_rate'], beta_1=config['train']['beta_1'],
        ...
        for mem in replay_buffer.sample():
            ...
            new_states = [np.array([memory_state.get_boolean_map()]).astype('float32'),
                          np.array([memory_state.movement_budget]).astype('float32')]
            hidden_state = network_model.representation(new_states)
            ...
            for start_index in range(sampled_index, sampled_index + num_unroll_steps):
                ...
                hidden_state, pred_reward = network_model.dynamic(
                    [hidden_state, mem.action_history[start_index]])
                #print(hidden_state, pred_reward)
                pred_policy, pred_value = network_model.prediction(hidden_state)
        ...
        loss += tf.reduce_sum(network_model.representation.losses) + tf.reduce_sum(network_model.dynamic.losses) + tf.reduce_sum(network_model.prediction.losses)  # regularization loss
