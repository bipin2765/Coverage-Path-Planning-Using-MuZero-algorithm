...
class BaseEnvironment:
    ...
    def step(self, state_, memory_: BufferMemory):
        ...
        reward = self.rewards.calculate_reward(state_, GridActions(action_index), current_state)
        action = np.array([1 if i==action_index else 0 for i in range(self.action_size)]).reshape(1,-1)

        memory_.state_history.append(current_state)
        ...
        self.stats.add_experience((state, action, reward, copy.deepcopy(current_state)))
        self.step_count += 1
